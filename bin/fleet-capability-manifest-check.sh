...
MANIFEST="/home/barry/projects/claudeteam/monitor-manifest.txt"
...
grep "^capability:" "$MANIFEST" 2>/dev/null | while read -r line; do
  cap=$(echo "$line" | cut -d: -f2 | xargs)
  type=$(echo "$line" | cut -d: -f3 | xargs)
  detail=$(echo "$line" | cut -d: -f4- | xargs)
  check_capability "$cap" "$type" "$detail"
done

for tool in curl jq bash systemctl; do
  check_capability "$tool" "tool"
done

check_capability "/home/barry/projects/claudeteam/rules.md" "file"
check_capability "/home/barry/projects/obsidian/00-Briefing.md" "file"
